...
def read_one_recipe_route(recipe_id):
    ...
    recipe = read_one_recipe(recipe_id)
...
def create_recipe_route():
    ...
    # fixed: call function directly; Blueprint objects don't expose module-level functions as attributes
    recipe_id = create_recipe(title, description, categoryid, authoruserid, ingredients, tags, dietaryflags, preptime, cooktime, servings)
...
def update_recipe_route(recipe_id):
    ...
    # fixed: same as above — call function directly
    result = update_recipe(recipe_id, title, description, categoryid, authoruserid, ingredients, tags, dietaryflags, preptime, cooktime, servings)
...
def update_many_recipes_route():
    ...
    # fixed: same as above — call function directly
    result = update_many_recipes(recipe_ids, title, description, categoryid, authoruserid, ingredients, tags, dietaryflags, preptime, cooktime, servings)
    # If no fields to update, return 400 error
    if result is None:
        return jsonify({"error": "No fields to update"}), 400
    # If no documents were modified, return 404 error
...
def delete_one_recipe_route(recipe_id):
    ...
    # fixed: same as above — call function directly
    result = delete_one_recipe(recipe_id)
...
def delete_many_recipes_route():
    ...
    # fixed: same as above — call function directly
    deleted_count = delete_many_recipes(recipe_ids)
    # If no documents were deleted, return 404 error
    if deleted_count == 0:
        return jsonify({"error": "No recipes found for the provided IDs"}), 404
